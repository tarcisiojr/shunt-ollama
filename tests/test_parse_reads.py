...
Os comandos abaixo vieram de sessões reais em que os hooks antigos falharam.
"""
...
class ParseReadsTest(unittest.TestCase):
    ...
    def test_pipe_head_limita(self):
        self.assertEqual(self.reads(f"cat {self.big} | head -40"), {self.big: [(1, 40)]})

    def test_sed_faixa(self):
        self.assertEqual(self.reads(f"sed -n '30,120p' {self.big}"), {self.big: [(30, 120)]})
        self.assertEqual(self.reads(f"sed -n 1,50p < {self.big}"), {self.big: [(1, 50)]})
        ...
        self.assertEqual(self.reads(f"sed 's/a/b/' {self.big}"), {self.big: [(1, None)]})
    ...
    def test_head_tail_variantes(self):
        ...
        self.assertEqual(self.reads(f"tail -20 {self.big}"), {self.big: [(-20, None)]})
        self.assertEqual(self.reads(f"tail -n +5 {self.big}"), {self.big: [(5, None)]})
        self.assertEqual(self.reads(f"head -c 5000 {self.big}"), {self.big: [(1, 100)]})

    def test_awk(self):
        self.assertEqual(self.reads(f"awk 'NR>=10 && NR<=50' {self.big}"), {self.big: [(10, 50)]})
        self.assertEqual(self.reads(f"awk '{{print}}' {self.big}"), {self.big: [(1, None)]})
    ...
    def test_rtk_e_wrappers(self):
        self.assertEqual(self.reads(f"rtk read {self.big}"), {self.big: [(1, None)]})
        self.assertEqual(self.reads(f"FOO=1 sudo cat -n {self.big}"), {self.big: [(1, None)]})
...
class RangeMathTest(unittest.TestCase):
    def test_merge_e_coverage(self):
        self.assertEqual(sc.merge_ranges([(1, 5), (4, 10), (12, 12)]), [(1, 10), (12, 12)])
...
class HookEndToEndTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {**os.environ, "SHUNT_MIN_LINES": "100", "SHUNT_EDIT_WINDOW": "80",
    ...
    def test_read_completo_nega_com_comando(self):
        out = self.run_hook("check-file-size", "Read", {"file_path": self.big})
        self.assertEqual(self.decision(out), "deny")
        self.assertIn("scripts/bulk-read", out["hookSpecificOutput"]["permissionDecisionReason"])

    def test_read_com_limit_grande_nega(self):
        out = self.run_hook("check-file-size", "Read", {"file_path": self.big, "limit": 620})
        self.assertEqual(self.decision(out), "deny")
    ...
    def test_bash_arquivo_pequeno_passa(self):
        out = self.run_hook("check-bash-read", "Bash", {"command": f"cat {self.small}"})
        self.assertEqual(self.decision(out), "allow")

    def test_fatiamento_acumulado_nega(self):
        out = self.run_hook("check-bash-read", "Bash",
                            {"command": f"sed -n '1,90p' {self.big}"})
        self.assertEqual(self.decision(out), "allow")
        out = self.run_hook("check-bash-read", "Bash",
                            {"command": f"sed -n '91,180p' {self.big}"})
        self.assertEqual(self.decision(out), "deny")
        self.assertIn("fatias", out["hookSpecificOutput"]["permissionDecisionReason"])
    ...
    def test_context_mode_batch_nega(self):
        out = self.run_hook("check-bash-read", "mcp__context-mode__ctx_batch_execute",
                            {"commands": [{"label": "x", "command": f"cat {self.big}"}],
                             "queries": ["a"]})
        self.assertEqual(self.decision(out), "deny")

    def test_context_mode_execute_python_passa(self):
        out = self.run_hook("check-bash-read", "mcp__context-mode__ctx_execute",
                            {"language": "python", "code": f"open('{self.big}').read()"})
        self.assertEqual(self.decision(out), "allow")

    def test_ollama_fora_libera(self):
        env = {**self.env, "OLLAMA_HOST": "http://127.0.0.1:9"}
        env.pop("SHUNT_ASSUME_OLLAMA")
        out = self.run_hook("check-bash-read", "Bash", {"command": f"cat {self.big}"}, env=env)
        self.assertEqual(self.decision(out), "allow")
